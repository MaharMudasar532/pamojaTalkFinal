import { Box, Tooltip, Typography, useTheme, IconButton, Grid, Modal, Button, Card, CardContent, Divider, Avatar } from "@mui/material";
...
import { Close, Cancel, Delete, Visibility } from "@mui/icons-material";
...
const Team = () => {
  ...
  const changeStatus = async (status) => {
    setIsloading(true);
    var InsertAPIURL = `${url}user/update_profile`
    var headers = {
      'Accept': 'application/json',
      'Content-Type': 'application/json',
    };
    let block_status = true;
    if (status === false) {
      block_status = "false";
    }
    var DataFinal = {
      ...
      .then(response => {
        ...
        if (response.status === true) {
          ...
          Swal.fire({
            icon: 'success',
            title: 'Success...',
            confirmButtonColor: COLORS.color2,
  ...
  const columns = [
    {
      field: 'user_name', headerName: <span style={{ color: "black", fontWeight: 600 }}>Username</span>,
      minWidth: 150,
    },
    {
    ...
    {
      field: 'date_of_birth', headerName: <span style={{ color: "black", fontWeight: 600 }}>DOB</span>,
      minWidth: 150,
    },

    {
      ...
      renderCell: (row) => {
        return (
          <>
            {row.row.block_status === "true" ?
              < Chip label='Block' color="success" variant="outlined" />
              :
              row.row.block_status === null ?
                < Chip label='unblock' color="success" variant="outlined" />
                :
                <Chip label='Unblock' color="primary" variant="outlined" />
          ...
          <Box width={{ xs: 400, md: 500, lg: 500, xl: 600 }} height="auto" sx={style}>
            <Grid container spacing={0}>
              ...
              <Grid xs={12} align="center" p={{ xs: 2, md: 5, lg: 1, xl: 1 }}>
                <Typography variant="h4" sx={{ letterSpacing: "3px" }} fontWeight={600} fontSize="x-large" color={COLORS.color2}>Confirmation</Typography>
                {DeleteData.block_status === true ?
                ...
                {DeleteData.block_status === 'true' ?
                  <>
                    {isloading ?
                      ...
                      <Button variant="contained" style={btn} onClick={() => { changeStatus(false) }}>unblock</Button>
                    }
                  </>
                  :
                  <>
                    {isloading ?
                      ...
                      <Button variant="contained" style={btn} onClick={() => { changeStatus(true) }}>block</Button>
                    }
                  </>
                }
              </Grid>
